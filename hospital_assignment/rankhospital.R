...
### step 7 : handle cases where the rank is too large, where the rank is "worst" or "best" 
...
rankhospital <- function(state,outcome,rank){
    ...
    #step 7
    num_entries <- nrow(ordered_hospitals)
    if(rank == "best") return(ordered_hospitals[1,1])
    if(rank == "worst") return(ordered_hospitals[num_entries,1])
    if(rank > num_entries) return(NA)
    else return(ordered_hospitals[rank,1])
